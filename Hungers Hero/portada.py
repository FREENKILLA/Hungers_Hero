# Importación de bibliotecas necesarias
import pygame
import cv2
from pygame.locals import QUIT
import subprocess  # Para ejecutar comandos externos

# Función para redimensionar una imagen manteniendo su relación de aspecto
def redimensionar_mantener_aspecto(src, ancho_objetivo, alto_objetivo):
    relacion_aspecto = src.shape[1] / src.shape[0]
    if ancho_objetivo / alto_objetivo < relacion_aspecto:
        w = ancho_objetivo
        h = int(w / relacion_aspecto)
    else:
        h = alto_objetivo
        w = int(h * relacion_aspecto)
    return cv2.resize(src, (w, h))

# Inicialización de Pygame
pygame.init()
ANCHO, ALTO = 1280, 720
pantalla = pygame.display.set_mode((ANCHO, ALTO))
pygame.display.set_caption('Hungers Hero')

# Configuración de la captura de video con OpenCV
cap = cv2.VideoCapture('img/VideosFondo/video.mp4')

# Configuración de sonidos con Pygame
pygame.mixer.init()
pygame.mixer.music.load('sounds/intro.mp3')
clic_sound = pygame.mixer.Sound('sounds/Seleccion de dificultad y play.wav')

# Definición de colores
MORADO = (120, 67, 230)
VERDE = (0, 255, 0)

# Configuración de fuente para texto
ruta_fuente = "fuentes/Anton-Regular.ttf"
fuente = pygame.font.Font(ruta_fuente, 50)

# Función para dibujar botones en la pantalla
def dibujar_boton(texto, x, y, pos_mouse):
    color = MORADO
    superficie_texto = fuente.render(texto, True, color)
    rectangulo_texto = superficie_texto.get_rect(center=(x, y))
    
    # Cambia el color si el ratón está encima del botón
    if rectangulo_texto.collidepoint(pos_mouse):
        superficie_texto = fuente.render(texto, True, VERDE)

    pantalla.blit(superficie_texto, rectangulo_texto)
    return rectangulo_texto

# Inicio del bucle principal del juego
ejecutando = True
reloj = pygame.time.Clock()
pygame.mixer.music.play(-1)  # Reproducción infinita del sonido

while ejecutando:
    # Captura del video frame a frame
    ret, fotograma = cap.read()
    if not ret:
        cap.set(cv2.CAP_PROP_POS_FRAMES, 0)
        ret, fotograma = cap.read()

    # Cambio de color de BGR a RGB
    fotograma = cv2.cvtColor(fotograma, cv2.COLOR_BGR2RGB)
    fotograma = redimensionar_mantener_aspecto(fotograma, ANCHO, ALTO)

    # Creación de una superficie Pygame desde una imagen OpenCV
    imagen_video_string = fotograma.tostring()
    superficie_video = pygame.image.fromstring(imagen_video_string, fotograma.shape[1::-1], 'RGB')
    
    # Dibujo del video en la pantalla
    pantalla.fill((0, 0, 0))
    desplazamiento_x = (ANCHO - fotograma.shape[1]) // 2
    desplazamiento_y = (ALTO - fotograma.shape[0]) // 2
    pantalla.blit(superficie_video, (desplazamiento_x, desplazamiento_y))

    # Detectar posición del ratón
    pos_mouse = pygame.mouse.get_pos()

    # Gestionar eventos de Pygame
    for evento in pygame.event.get():
        if evento.type == QUIT:
            ejecutando = False
        elif evento.type == pygame.MOUSEBUTTONDOWN:
            # Comprobaciones para detectar en qué botón se hizo clic
            if boton_reproducir.collidepoint(pos_mouse):
                clic_sound.play()
                subprocess.Popen(["python", "portada2.py"])
                print("¡Play clickeado!")
            elif boton_informacion.collidepoint(pos_mouse):
                clic_sound.play()
                subprocess.Popen(["python", "portada3.py"])
                print("¡Información clickeado!")

    # Dibujo de botones en pantalla
    boton_informacion = dibujar_boton('Informacion', ANCHO/2, ALTO/2 - 30, pos_mouse)
    boton_reproducir = dibujar_boton('Jugar', ANCHO/2, ALTO/2 + 30, pos_mouse)

    # Actualización de la pantalla
    pygame.display.flip()
    reloj.tick(30)

# Finalización del juego
pygame.mixer.music.stop()
cap.release()
pygame.quit()
